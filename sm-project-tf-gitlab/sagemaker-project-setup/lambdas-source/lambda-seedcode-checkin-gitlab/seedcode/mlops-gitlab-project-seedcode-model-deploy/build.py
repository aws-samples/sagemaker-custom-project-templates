...
def get_approved_package(model_package_group_name):
    ...
    # Return the model package arn
        ...
        #Describe latest model package
        model_description = sm_client.describe_model_package(ModelPackageName=model_package_arn)
            
        #Extract the model output
        registry_artifact = model_description['InferenceSpecification']['Containers'][0]['ModelDataUrl']
        model_image_uri = model_description['InferenceSpecification']['Containers'][0]['Image']
        
        # print(f'Model description is :{model_description}')
        print(f'Model Image URI is present in :{model_image_uri}')
        print(f'Model artifact is present in :{registry_artifact}')
        ...
        return model_package_arn, model_image_uri, registry_artifact
...
def extend_config(args, model_package_arn, stage_config,model_image_uri, registry_artifact):
    """
    ...
    new_params = {
        ...
        "ModelImageURI": model_image_uri,
        "ModelRegistryArtifact": registry_artifact,        
    }
    new_tags = {
...
if __name__ == "__main__":
    ...
    model_package_arn, model_image_uri, registry_artifact = get_approved_package(args.model_package_group_name)

    # Write the staging config
    with open(args.import_staging_config, "r") as f:
        staging_config = extend_config(args, model_package_arn, json.load(f),model_image_uri, registry_artifact)
    ...
    with open(args.import_prod_config, "r") as f:
        prod_config = extend_config(args, model_package_arn, json.load(f),model_image_uri, registry_artifact)
