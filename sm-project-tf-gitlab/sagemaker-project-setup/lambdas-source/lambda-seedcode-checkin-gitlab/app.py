...
def lambda_handler(event, context):
    ...
    gitlab_build_repo_branch = os.environ["GitlabBuildRepoBranch"]
    gitlab_deploy_repo_branch = os.environ["GitlabDeployRepoBranch"]
    ...
    try:
        ...
        for deploy_project in deploy_project_list_resp:
            deploy_proj_name = deploy_project.name
            deploy_proj_name_with_ns = deploy_project.name_with_namespace

            deploy_project_list.append(deploy_proj_name)
            if deploy_proj_name == gitlab_project_name_deploy:
    ...
    try:
        deploy_project.variables.create(
            {
                "key": "SAGEMAKER_PROJECT_NAME",
                "value": os.environ["SageMakerProjectName"],
            }
    ...
    build_data = {
        "branch": gitlab_build_repo_branch,
        ...
        for name in files:
            ...
            if name.startswith("._"):
                ...
                try:
                    ...
                    deploy_data["actions"].append(deploy_action)
                except:
                    pass
