...
def get_pipeline(
    ...
    processing_image_name = "mlops-{0}-processing-dev".format(project_name)
    training_image_name = "mlops-{0}-training-dev".format(project_name)
    inference_image_name = "mlops-{0}-endpoint-dev".format(project_name)
    ...
    try:
        ecr.describe_images(
            repositoryName=training_image_name, imageIds=[{"imageTag": "latest"}]
        )
        training_image_uri = (
            f"{account_id}.dkr.ecr.{region}.amazonaws.com/{training_image_name}:latest"
        )
    except:
    ...
    try:
        ecr.describe_images(
            repositoryName=inference_image_name, imageIds=[{"imageTag": "latest"}]
        )
        inference_image_uri = (
            f"{account_id}.dkr.ecr.{region}.amazonaws.com/{inference_image_name}:latest"
        )
    except:
